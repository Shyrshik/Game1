...
{                   // namespace с одинаковым именем можно использовать не указывая в using. Классы в namespace только public.
    ...
    /// <summary>
    /// Имя класа должно быть таким же как имя файла.
    /// </summary>
    public class Classes    // Класс - это свой тип данных.
    {
        ...
        /// <returns></returns>
        public void Start(int c)        // Метод обьекта класса.
        {
            Classes a = new Classes();  // Создание обьекта или экземпляра класса.
            Classes b = new();          // То же самое, что и выше.
        }

        //Модификаторы доступа
        private int i1;             // Виден только внутри этого обьекта.
        ...
        /// Структура метода: <возвращаемый тип> <имя> (<параметры>) тип void означает "Ничего" (как процедура).
        /// По умолчанию область видимости (контекст) у свойств и методов Private(только внутри класса).
        static void Context(int c)  // Статический метод можно вызывать не создавая обьект класса. Статические методы не доступны
        {                           // экземплярам(обьектам) класса. Не могут взаимодействовать с нестатическими свойствами и метадами.
            {
                float b = 1;        // Контекст переменных, обьявленных в медодах, ограничен логическими скобками
                                    // в которых их создали(обьявили).
            }
            ...
            classes.a = 1;          // Так обращаемся к нестатичной переменной, создав экземпляр обьекта.
        ...
        {
            this.f = f;             // this.f свойство ОБЬЕКТА КЛАССА(с_Script.f), f - параметр метода Context.(только в !static). 
                                    // this - это ссылка на обьект класса.
            Classes.e = 1;          // обращение к статическому свойству обьекта класса. 
            float h = 1f;
            Ref(ref f,              // Передаем ссылку на переменную, а не значение. Ref не принимает не идентифицированные переменные.
            out float g,            // В out можно передать не идентифицированные переменные, out обязательно приcваивают значение.
        ...
        void Param(int g, params float[] a)     // params принимает любое колличество параметров типа float через запятую.     
        {                                       // или один массив float[]. Должен стоять последним в списке параметров.       
            Method(k: 3, j: 1);                 // Именнованные параметры. Можно передавать не по порядку,                        
                                                // если указать какой параметр передаем.                                          

            Classes c_s = new Classes()         // Обьявление переменных. (инициализация класса)
            {
                a = 34,
                f = 10
            };
        }
        ~Classes() { }              // Фанлайзер??? Диструктор??? Вызывается при уничтожении класса сборщиком муссора.
    ...
    public static class Statics     // Статический класс. Нльзя создать экземпляр(обьект) статического класса.
    {                               // Статический класс может содержать только статические конструкторы, свойства и методы.
        ...
        static void M1() { }    // Статический метод, для его вызова не нужно создавать обьект класса (обьектам недоступны). 
                                // Все созданные обьекты класса ссылаются на одно статическое свойство!
                                // Не могут взаимодействовать с нестатическими свойствами и метадами.

        // Extension методы (методы расширения). Служат для добавления в клас дополнительных методов.
        // Должны быть статическими и находиться в статическом классе, контекст public.
        // В параметрах первым указываем текущий экземпляр(обьект) класса. this <тип_данных_для_которого_этот_метод> <имя_любое>
        ...
        {
            aa = new Classes();
            aa.Mmm(2);               //Обращение к расширению через экземляр класса.
        }
    }
    ...
    {
        // Конструкторы классов при создании экземпляра дочернего создают сначало радительские, в которых так же срабатывает конструктор!
        public Classes1() : base(3f) { }    // Указываем параметры для конструктора родительского (базового) класса
        public new void Start(int a1)       // Скрываем одноименный метод базового класса. При вызове из обьекта класса будет срабатывать
        {                                   // метод из БАЗОВОГО класса, чтобы срабатывал этот метод используют virtual и override.
            ...
            Classes a = new Classes1();             // В родительский клас можно присваивать класс унаследованный, с потерей данных из
                                                    // наследника.

            Classes1 b = (Classes1)new Classes();  // В дочерний клас нельзя присвоить родительский без явного преобразования типов.


        }
        protected virtual void Virtuals() { }   // Виртуальный метод можно переопределить в дочернем классе. Не мможет быть private.
    }
    class Classes2 : Classes1
    {
        protected override void Virtuals()      // Переопределение метода из базового класса. При вызове из обьекта класса будет 
    ...
    abstract class Abstracts            // Для абстракттного класса нельзя создать обьект (экземпляр). От них можно только наследоваться.
    {                                   // Абстрактный клас - это описание шаблона, который  обязательно должен реализовать наследник.
        protected abstract int A { get; }      // Абстрактное поле.
        protected abstract void Absd(); // Абстрактный класс не может иметь тело с коммандами. Не мможет быть private. Наследные классы
                                        // должны обязательно реализовать абстракиные методы.
    }
    class Abstracts1 : Abstracts
    {
        ...
        protected override void Absd()                  // Переопределение метода из базового класса. При вызове из обьекта класса будет 
        {                                               // срабатывать этот метод.( для virtual и abstract)
...
class Interfaces : IInterfaces, IInterfaces1     // Реализация интерфейсов в классе, интерфейсы поддерживают множественное наследование.
{
    ...
    {
        IInterfaces a = new Interfaces();    // Переменной типа интерфейса можно присвоить класы которые этот интерфейс реализуют.
...
{
    public int a;
    public Recursions Recr;     // Можно определить свойство самого себя, в дальнейшем его обходить рекурсивно c проверкой null.
    void Recursiya()            // Рекурсия это метод, вызывающий сам себя.
    {
        // Рекурсии ограничены размером стэка (Stack). Каждый новый вызов занимает место в стеке.
        Recursiya();
    }
}
public class RecursionsBypass
{
    ...
    void Main()
    {
        ...
        r.a = 2;
        r.Recr = new Recursions();
        r.Recr.a = 4;
        r.Recr.Recr = new Recursions();
        r.Recr.Recr.a = 6;
        Bupass(r);
        BupassByLoops(r);
        Console.ReadLine();
    }
    void Bupass(Recursions re)              //Обход рекурсией
    {
        Console.WriteLine(re.a + " ");      // Выведет "2 4 6"  
        if (re.Recr == null) return;
        Bupass(re.Recr);
    }
    void BupassByLoops(Recursions re)       //Обход циком.
    {
        for (Recursions i = re; i != null; i = i.Recr)
        {
...
public class GettersAndSetters
{
    ...
    // поэтому создаем отдельную приватную переменную в которую все записываем,и используюем ее только через метод геттера сеттера
    ...
    // отображение для гетеров сетеров в inspector unity (при условии что нет логики)
    [field: SerializeField] private float N { get; set; }

    private GameObject _prefab;
    public GameObject Prefab { get => _prefab; set => _prefab = value; }
    // ////////////////////////////////////////////////////////////////////////////////////////////
    // Ввести "prop" => tab => tab  : Visual Studio напишет геттерсеттер по умолчанию.
    public int MyProperty1 { get; set; }

    // Ввести "propa" => tab => tab  : Visual Studio напишет геттерсеттер по умолчанию.
    public int MyProperty2 { get; private set; }

    // Ввести "propfull" => tab => tab  : Visual Studio напишет геттерсеттер по умолчанию.
    private int myVar;
...
public class Variables
{
    private Variables()
    {
        ...
        // При выходи из контекста уничтожается переменная со ссылкой, но клас остается в куче(в памяти), пока сборщик мусора его не удалит.

        int h1 = 10;
        int h2 = 0b101;     // бинарная форма b = 5
        int h3 = 0xFF;      // шестнадцатеричная форма c = 255
        int? b0 = null;     //int? - если присваевается null, то в переменную записывается значение по умолчанию (для int = 0)
        ...
        dynamic dy = 5;     // Нетипизированная переменная. !!! Нахуй ее, никогда не используй.!!!
        ...
        int[] a = new int[] { 1 };  //a присваевается ссылка на массив в куче(Heap).
        ...
        int[] e = { 1, 2, 3 };      //e присваевается ссылка на другой массив в куче(Heap).    
        ...
        string s = null;   //null - По умолчанию в ссылочных типах(Reference types) означает пустая ссылка.
                           //(обьявили но не инициализировали).Нельзя присвоить значимым типам (Value types).
    }
}
//Variables.                                                                                                                      Variables.
//Operators.                                                                                                                      Operators.
...
public class Operators : MonoBehaviour
{
    Operators(int a, int g, bool d = true, string str = "")
    {
        ...
        print(a++);     // Cначала print(a), потом a++ (выведет 5). (return a; a+=1;)
        ...
        print(++a);     // Cначала ++a, потом print(a) (выведет 7). (a+=1; return a;)
        ...
        // & и | - одинарные тоже можно использовать, но 2е более эффективны, в двойных вычисляются выражения поочереди,
        // и если по первому выражению ясен результат дальше вычисление прерывается для быстродействия.
        if (d && d) { } // Cокращенное И.
        if (d || d) { } // Cокращенное ИЛИ.
        if (!d) { }     // НЕ.
        ...
        m = (int)n;     // Явное преобразование.(explicit converting). Если значение больше чем может хранить преобразующий тип,
                        // то данные потеряются ( после максимального значения отсчет начнеться сначала).
                        //using System;
        ...
        bool s = float.TryParse("12", out float q);  // Преобразует строку в тип float (либо можно в другие).
                                                     // Если не сможет преобразовать - вернет false, а в q запишит значение по умолчанию.
        object ob1 = 24;
        ...
        if (op2 is Operators op3) { }       // Проверяет у op1 можно ли неявно приобразовать в Operators, если да то op3 = op2,
...
public class ConditionalOperators
{
    ConditionalOperators(int a = 0, int g = 1, bool d = true)
    {
        ...
        s = s ?? string.Empty;  //=""; ?? - Это ператор null обьединения. Проверяет s, если null то присваевает ""
        s ??= "abc";            //=""; s==null ? "" : s ; string.Empty = ""
        GameObject go = null;   //!!! для GameObject null обьединения работают через раз, и unity не советует их использовать. Проверил на 
        ...
        int? j = s?.Length;     //?. - Оператор условного null. если s==null то дальше вычисления прекратит и вернет null вместо ошибки. 
                                //int? - если присваевается null, то в переменную записывается значение по умолчанию (для int = 0)

        bool t = j.HasValue;    //=false; Если значение null - возвращает false.
        int i = j.Value;        //=ошибка; Надо сначало проверить на null j, если j.HasValue==false, то в j.Value нет значения.
        ...
        {
            case 1:             // if (a == 1) 
                goto case 4;    // Выполняет операторы из выбраного case без проверки условий.
...
public class Arrays
{
    Arrays()
    {
        float[] a = new float[3];           // Cоздаем пустой массив, инициализируем и присваеваем ему массив с 3 эллементами.
                                            // Значение элементов заполняются значениями по умолчанию в зависимости от типа (у int это 0).
                                            // new float[3] - размер указывать обязательно, если не присваеваем значения в фигурных скобках.
        ...
        a = new float[3] { 4f, 3f, 3f };    // Присваеваем новый массив с эллементами { 4f, 3f, 3f };
        a = new float[] { 4f, 3f, 3f, 5f }; // Если не указать размер, то создасться масив с колличеством эллементов в фигурных скобках
                                            // (в данном примере 4).
        a = new[] { 4f, 3f, 3f, 5f };       // Тоже самое, что и выше.
        float[] a1 = { 0f, 3f, 3f, 5f };    // Тоже самое, что и выше, работает только при обьявлении переменной.
        a[2] = 3f;                          // Присваеваем 2му элементу значение 3f в одномерном массиве.
        aa[2, 2] = 3f;                      // Присваеваем элементу 2,2 значение 3f в многомерном массиве.
        ab[2][2] = 3f;                      // Присваеваем элементу 2,2 значение 3f в зубчатом массиве.
        a[^2] = 3f;                         // Присваеваем предпоследнему элементу значение 3f.
        ...
        a[ind] = 3f;                        // Присваеваем предпоследнему элементу значение 3f.
        ...
        a = new[] { 0f, 1f, 2f, 3f };
        int b1 = a.Length;                          // = 3; Возвращает колличество эллементов массива (только чтение)
                                                    // (в многомерных массивах возвращает колличество из всех измерений массива).
        int b2 = aa.GetLength(0);                   // = 3; Возвращает колличество эллементов массива в определенном измерении
                                                    // (нумерация начинается с нуля).
        Array.Reverse(a);                           // = { 3f, 2f, 1f, 0f }; Переворачивает массив, первые эллементы становятся последними.
        Array.Sort(a);                              // = { 0f, 1f, 2f, 3f }; Сортирует (массив а) в порядке возрастания.
        float f08 = Array.Find(a, i => i > 0);      // = 1f; Ищет в массиве первый эллемент по условию. (с начала по возрастанию).
        float f09 = Array.FindLast(a, i => i < 3);  // = 2f; Ищет в массиве последний эллемент по условию.
        float[] f10 = Array.FindAll(a, i => i < 3); // = { 0f, 1f, 2f }; Возвращает массив с эллементами подходящими по условию.
                                                    // (с начала по возрастанию, не сортирует).
        int f11 = Array.FindIndex(a, i => i < 3);   // = 0; Возвращает индекс первого эллемента подходящего по условию.
                                                    // (с начала по возрастанию). (-1 возвращает если не нашел)
        int f12 = Array.FindLastIndex(a, i => i < 3);   // = 2; Возвращает индекс последнего эллемента подходящего по условию.
                                                        // (-1 возвращает если не нашел)
        int f13 = a.Rank;                           // = 1; Возвращает колличество измерений массива.

        //using System.Linq;
        a = Enumerable.Repeat(3f, 4).ToArray();     // = { 3f, 3f, 3f, 3f}; Создает массив размером 4, все эллементы массива заполняются
                                                    // значением 3f.
        int[] c = Enumerable.Range(3, 4).ToArray(); // = { 3, 4, 5, 6}; Создает массив int[] размером 4, эллементы массива заполняются
                                                    // значенями от 3 прибовляя по 1.
        a = new[] { 0f, 1f, 2f, 3f };
        float f01 = a.Max();                        // = 3f; Возвращает максимальное значение.                
        float f02 = a.Min();                        // = 0f; Возвращает минимальное значение. 
        float f03 = a.Sum();                        // = 6f; Возвращает сумму эллементов массива.
        float f04 = a.Where(i => i % 2 != 0).Sum(); // = 4f; Where отбирает элементы по условию.
        float[] f05 = a.Distinct().ToArray();       // = { 0f, 1f, 2f, 3f }; Возвращает массив, в котором убирает повторяющиеся эллементы.
        ...
        for (int i = 0; i < a.Length; i++)      // Обход эллементов массива
        {
            float d = a[i];
        }

        //using System.Collections.Generic;
        List<float> values = new List<float>();     // ??? Какой то продвинутый массив, надо заагуглить про него.???
        ArrayList va = new ArrayList();             // ??? Может содержать любой тип данных ??? (сейчас не используют). То же самое, что и
                                                    // List<>, только работает с обьектами вместо Generics, поэтому медленнее в сотни раз.
    }
}//Arrays.                                                                                                                           Arrays.
//Enums.                                                                                                                              Enums.
...
// Атрибуты размещаются перед обьектами, на которые они нацелены.

//using System;
[Serializable]                              // Сериализует сразу весь класс или структуру (выводит все из него в inspector unity).

//using UnityEngine;
[RequireComponent(typeof(Rigidbody))]       // Проверяет есть ли компонент на обьекте, если нету то добавляет его.
[DisallowMultipleComponent]                 // Запрещает добавлять больше одного компонента данного класа на обьект.
[SelectionBase]                             // выбирает обьект целиком, при нажатии на любую его часть или дочерний обьект.
[HelpURL("https://")]                       // справка по классу, открывается при нажатии в inspector unity на вопросик.

// Обьект класса можно создать из меню Create в папке асетов. (Для создания ScriptableObject), order - позиция в списке?
[CreateAssetMenu(fileName = "C_script", menuName = "C_Script/New", order = 10000)]

//[DeScriptExecutionOrder(1)]               // ???Якобы назначение очереди загрузки скриптов, не работает чето.
...
public class Attributes : ScriptableObject  //ScriptableObject нужен для CreateAssetMenu
{
    ...
    [field: SerializeField] private float N { get; set; }    // Отображение для гетеров сетеров (при условии что нет логики).
    ...
    [Tooltip("Подсказка")] float re;        // Подсказка в inspector unitu отображается при наведении мыши на поле.
    [Range(-100f, 100f)] public float we;   // Ограничение значений переменной, в inspector unity появляется ползунок.
    [Min(1f)] public float wd;              // Уcтанавливает минимальное значение.
    [ContextMenu(nameof(M1))] private int M1() => 5;    // Добавляет метод в режиме едитора в контекстное меню свойства.
    [Multiline(5)] private string dje;      // Поле с фиксированным колличесвом строк inspector unity, без скрола. Ширина в пол инспектора.

    // Строка отображается полем с количеством строк от 1 до 3 в inspector unity, есть скрол. ширина поля во весь инспектор.
    ...
    private enum Enum                       // Перечисления???
    {
        ...
        [InspectorName("Fdfasdf")]
        Eeee = 1, wwww = 2
    }

    //using UnityEngine.Serialization;
    [FormerlySerializedAs("hjj")] private int w;    // Сохранить старое название когда переименовываешь переменную в прифабе????
}
...
public class OrderOfMethods : MonoBehaviour
{
    ...
    void Awake() { }        // "Эта функция всегда вызывается до начала любых функций, а также сразу после инициализации префаба."
                            // Но чето нихуя, она не выполнилась до старта в другом скрипте.
    ...
    void OnTriggerEnter2D(Collider2D other) { } // При входе в 2D тригер.
    void OnTriggerEnter(Collider other) { }     // При входе в тригер.

    void OnTriggerStay2D(Collider2D other) { }  // При нахождении в 2D тригере.
    void OnTriggerStay(Collider other) { }      // При нахождении в тригере.

    void OnTriggerExit2D(Collider2D other) { }  // При выходе из 2D тригера.
    void OnTriggerExit(Collider other) { }      // При выходе из тригера.

    private void OnEnable() { }                 // При включении обьекта.
    private void OnDisable() { }                // При отключении обьекта.
    private void OnDestroy() { }                // При уничтожении обьекта.

    void OnMouseDown() { }                      // Вызывается при щелчке мышкой по обьекту.
...
public class OldInputSystem
{
    OldInputSystem()
    {
        ...
        bool d2 = Input.GetKeyDown(KeyCode.K);   // Проверяет нажата ли клавиша. True - только в момент нажания клавиши.
        ...
        {
            // В коллекции Input.touches храняться все действующие на данный момент переменные Touch,
...
public class NewInputSystem
{
    ...
    // .. .. .. .. .. .. .. .. .. .. .. .. .. .. .. .. .. .. .. .. .. .. .. .. .. .. .. .. .. ..
    //создаем переменную класса.(InputСontroller - это скомпилированный Input Actions,
    //после всех настроек создается класс с именем как вы назвали.)
    private InputСontroller _inputСontroller;
    private void Awake()
    {
        _inputСontroller = new();         // создаем экземпляр класса, обязательно в Awake(), что бы до OnEnable();
    }
    private void OnEnable()             //Активируем систему при включении обьекта.          
    {
        _inputСontroller.Enable();
    }
    private void OnDisable()            // Деактивируем при отключении обьекта.
    {
    ...
    NewInputSystem()
    {
        ...
        _inputСontroller.Player.Run.performed += context => MoveObject();

        // обработка отпускания кнопки.
        _inputСontroller.Player.Run.canceled += context => MoveObject();

        // получение переменной, если возвращаемый тип выбран в Input Actions.
        Vector2 _movePlayer = _inputСontroller.Player.Move.ReadValue<Vector2>();
    }
}
*/
...
public class Vectors
{
    Vectors()
    {
        ...
        Vector2 vi = v1.normalized;     // = (x1/v1.magnitude, y1/v1.magnitude); - Возвращает вектор с тем же направдением, 
                                        // но длиной 1.(Только чтение).
        v1.Normalize();                 // = (x1/v1.magnitude, y1/v1.magnitude); - Записывает сам в себя вектор с тем же направдением,
        ...
        /// Возвращает вектор совпадающий с вектором v1, но с длиной b. Eсли длина b больше длины вектора v1, то возвращается v1.
        ...
        Vector2 vk = Vector2.Lerp(v1, v2, b);            // = (2,1); Линейная интерполяция(Движение к обькту).
        ...
        /// b=0.5 вернет биссиктрису между векторами которая будет лежать на прямой соеденяющей векторы v1 и v2.
        ...
        /// Тоже, что и Lerp, только вместо пропорции b указывается расстояние, используется для перемещения обьектов друг к другу.
        ...
        /// Тоже, что и Lerp, но продвинутый с ограничением скорости, плавное движение к обьекту со временем, 
        /// в переменную va записываются промежуточные данные от кадра к кадру. 2f - время движения к обьекту, если будет слишком большое 
        /// то обьект сначало будет уходить а потом возвращаться. 1f - максимальная скорость движения обьекта.
    }
}
...
public class RigidBody2Ds : MonoBehaviour
{
    RigidBody2Ds()
    {
        ...
        _rigidBodyThis = GetComponent<Rigidbody2D>();               // получить компонент с обьекта
        ...
        /// Игнорит колайдеры (проходит сквозь стены), отталкивает Dynamic обьекты от себя. (неподвижный объект с бесконечной массой)

        _rigidBodyThis.bodyType = RigidbodyType2D.Static;           // Неподвижный обьект, взаимодействует только с Dynamic
        /// (если не добавлять Rigidbody2D, но добавить в обьект колайдеры - 
        ...
        _rigidBodyThis.simulated = true;                            // Собственно включение/отключение всей физики, колайдеров.
        /// при отключеном калайдеры ни с чем не взаимодействуют вообще, будто их и не ставили).
        ...
        _rigidBodyThis.useAutoMass = false;                         // Масса заависит от размера коллайдера.
        ...
        _rigidBodyThis.gravityScale = 0f;                           // Степень воздействия гравитации на обьект
        /// (ускорение сободного падения? или множитель ускорения свободного падения?).
        /// /////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        /// //////////////////////////////////////////////////////////////////////////////////////////////////////////////
        // Sleeping Mode (Режим снаа) типа отключает у обьекта Rigidbody2D что бы не напрягать процессор, пока с обьектом что либо
        // не произойдет, (на практике хуй пойми что делает - никаких изменений).
        // (в описании: когда обьект не двигается, расчет физики для него игнорируется, пока не начнет движение или не заденут коллайдер).
        _rigidBodyThis.sleepMode = RigidbodySleepMode2D.NeverSleep;     // Режим никогда не спит.(рекомендуется не использовать)
        _rigidBodyThis.sleepMode = RigidbodySleepMode2D.StartAwake;     // Обьект появляется на сцене не спящим.
        _rigidBodyThis.sleepMode = RigidbodySleepMode2D.StartAsleep;    // Обьект появляется на сцене спящим.
        _rigidBodyThis.Sleep();                                         // Отправляет Rigidbody2D в сон.
        _rigidBodyThis.IsSleeping();                                    // Возвращает значение, спит обьект или нет.
        /// //////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        _rigidBodyThis.interpolation = RigidbodyInterpolation2D.Interpolate;    // Движение сглаживается на основе позиций обьекта
                                                                                // в предыдущих кадрах.
        _rigidBodyThis.interpolation = RigidbodyInterpolation2D.Extrapolate;    // Движение сглаживается на основе позиций обьекта
                                                                                // в следующем кадре.
        /// //////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
public class ScriptableObjects : ScriptableObject
{
    ...
    void SO()
    {
        ...
        AssetDatabase.CreateAsset(a, "Assets/" + a.N + ".asset");                   // Сохранение в ассеты.
        a.N = "b";                                                       // Изменения будут только во время выполнения игры,
                                                                         // после перезахода они исчезнут.

        AssetDatabase.SaveAssets();                                     // НЕ записывает ассеты на диск.!!!
        ...
        AssetDatabase.CreateAsset(a, "Assets/rr.asset");                //a.name = "rr"; записали новый, имя файлы перезаписалось.
...
//ScriptableObjects.                                                                                                      ScriptableObjects.
//UnitysOthers[0].                                                                                                          UnitysOthers[0].

//using System;
//using UnityEngine;
//using UnityEngine.SceneManagement;

public class UnitysOthers : MonoBehaviour
{
    // Другие фишки Unity.
    void ForStartMethod()
    {
        ...
        // Получение ссылок на компоненты объктов.
    ...
    void MoveObject()
    {
        // transform вызывается без GetComponent, так как есть во всех обьектах. Для других компонентов доcтуп только через GetComponent.
    ...
    {
        // разрушить обьект, в данном случае который коснуулся тригера
    ...
    void Prefabs()
    {
        ...
        // Префабы, программное размещение
    ...
    void Invoke()
    {
        ...
        bool a = IsInvoking(nameof(Destroy));           // Проверяет отложеный запуск для метода Destroy
    }
    ...
    void LoadScene()
    {
        ...
        DontDestroyOnLoad(this);            // Не разрушать объект при загрузке другой сцены (перенести его на следующий уровень).
                                            // this - означает этот обьект, либо можно указать любой другой.

        // PlayerPrefs - обьект для сохранения и загрузки данных.
    ...
    void Draw()
    {
        ...
        Debug.DrawLine(begin, end, Color.green);    // Рисует линию в голобальных координатах от begin до end.
...
//  git merge Name              // Обьеденяет текущую ветку с веткой Name.
//  git rebase Name             // Переносит текущий commit в ветку Name.
//      // "HEAD" - выбранный в данный момент commit, с которым вы сейчас работаете. Всегда указывает на последний коммит в дереве  
...
//  git checkout Name^^         // Смещает "HEAD" на 2 commit'а с конца в ветке Name.
//  git checkout Name~3         // Смещает "HEAD" на 3 commit'а с конца в ветке Name.
//  git checkout HEAD^          // Смещает "HEAD" на предпоследний commit относительно текущего положения.
//  git branch -f Name HEAD~3   // Сместить указатель ветки Name относительно "HEAD" на 3 commit'а относительно текущего положения.
//
//  git reset HEAD^             // (отмена изменений) Сместит текущую ветку на 1 commit, как будто отмененного commit'а не было.
...
//
//  git cherry-pick <Commit1> <Commit2> <...>   // Копирует commit'ы относительно HEAD, ставит их следующими. commit'ы писать через пробел.
//  git rebase -i HEAD~3        // Открывает окно в котором можно вручную редактировать 3 комита относительно HEAD.
//  
//
...
// DRY Принцип не повторяйся (dont repeat yourself) = SSOT
// Данные в системе долны иметь только одно представление. 
// SSOT Принцип единственный источник истины (single source of truth (SSOT) = DRY
// Не далать дублирование данных.
...
// Не делай то что сейчас не нужно. Не надо на будущее реализовывать лишнии функции или интерфейсы.
